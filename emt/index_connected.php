...
function fetchAllPoints(id_releve, callback) {
    const url = 'getpointsgps.php?id_releve=' + id_releve;
...
var pathCoordinates = []; // Tableau pour stocker les coordonnées des points
var relevepath; // Variable pour stocker la polyline

function addPointToMap(point, index, totalPoints) {
    var lat = point.LATITUDE;
    var lon = point.LONGITUDE;

    if (lat != 0 && lon != 0) {
        // Set markers for the first and last point with popup text
        if (index === 0) {
            L.marker([lat, lon]).addTo(myLayerGroup).bindPopup('Début');
            pathCoordinates.push([lat, lon]); // Ajouter les coordonnées du premier point au tableau
            relevepath = L.polyline(pathCoordinates, { color: 'green' }).addTo(myLayerGroup); // Initialiser la polyline avec les coordonnées du premier point
        } else if (index === totalPoints - 1) {
            L.marker([lat, lon]).addTo(myLayerGroup).bindPopup('Fin');
        } else {
            // Ajouter les coordonnées du point à relier au tableau
            pathCoordinates.push([lat, lon]);
            // Mettre à jour les coordonnées de la polyline avec les nouveaux points
            relevepath.setLatLngs(pathCoordinates);
        }
        // Décocher la case à cocher et arrêter l'intervalle d'auto-rechargement
        $("#jqxCBAutoReload").jqxCheckBox('uncheck');
        clearInterval(refreshIntervalId);
        // Center and zoom on the current point
        mymap.setView([lat, lon], 17);
    }
}


// Function to display points one by one with a delay
function displayNextPoint(allPoints, currentIndex) {
    var interval = parseInt($('#Interval').val());
    if (currentIndex < allPoints.length) {
        console.log('Displaying point at index: ' + currentIndex); // Debugging
        addPointToMap(allPoints[currentIndex], currentIndex, allPoints.length); // Add point to map
        currentIndex++;
        setTimeout(function() {
            displayNextPoint(allPoints, currentIndex);
        }, interval);  // Delay of interval ms between points
    }
}
...
function clearMap() {
    pathCoordinates = []; // Tableau pour stocker les coordonnées des points
    relevepath = null;
    mymap.removeLayer(myLayerGroup); // Supprimer toutes les couches de la carte
    myLayerGroup = new L.layerGroup().addTo(mymap); // Réinitialiser la couche de groupe
    // Décocher la case à cocher et arrêter l'intervalle d'auto-rechargement
    $("#jqxCBAutoReload").jqxCheckBox('uncheck');
    clearInterval(refreshIntervalId);
}
...
$(document).ready(function () {
    ...
    // Ajouter et configurer le bouton jqxButton
    $("#replayButton").jqxButton({ width: '200px', height: '30px' });

    $("#replayButton").on('click', function () {
        const id_releve = $('#ddl_releves').val();
        clearMap();
        fetchAllPoints(id_releve, function (allPoints) {
            displayNextPoint(allPoints, 0);
        });
    });
    ...
    $("#Interval").jqxInput({ placeHolder: "Enter text",  height: 20, width: 50,
...
?>
    <div class="parent" style="margin-top: 5px;">
        Nombre de lignes du tableau :	<input type="text" id="LimitField"/>&nbsp;&nbsp;<div style="margin-top: 5px;" id="jqxCBAutoReload">Refresh</div>
    </div>
...
<!--    </div>-->
    ...
    <div class="parent" style="margin-top: 5px;">
        <input type="button" value="Rejouer ce relevé" id='replayButton' />
